...
class RecentReviews extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {reviews: [], currentUrl: props.url, nextUrl: null, previousUrl: null, loading: false };
    }
    fetchReviews(){
        ...
        fetch(this.state.currentUrl, {
            ...
            this.setState( { loading: false, reviews: data.results, nextUrl: data.next, previousUrl: data.previous})
        })
    }
    componentDidMount() {
        this.fetchReviews()
    ...
    loadPrevious() {
        ...
        this.state.previousUrl = this.state.previousUrl;
        this.fetchReviews();
     }
    ...
    render() {
        ...
        const previousButton = <button onClick={ () => {this.loadPrevious()} }
                          className="btn btn-secondary"
                          disabled={this.state.previousUrl == null }>
                          Previous
                          </button>;

        const nextButton = <button onClick={ () =>{this.LoadNext() } }
